...
// Purpose : Computing and Displaying Edge Map
...
int main( int argc , char** argv )
{
	...
	cvResizeWindow("EdgeMap", width , height );
	...
	for( int imgCount = imgStartNum ; imgCount <= imgEndNum ; ++imgCount )
	{
		...
		unsigned char erc = computeEdgeMap( rgbImg , monImg );
		if( erc != 0 )
		{
			printf( "\n\n Error Code = %d \n\n" , erc );
			getchar();
