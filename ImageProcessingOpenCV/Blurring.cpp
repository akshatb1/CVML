...
int main( int argc , char** argv )
{
	...
	int blurWindowSize;
	fscanf( fp , "%s%s%d" , paramNameString , eqString , &blurWindowSize);
	...
	IplImage* blurImg = NULL;
	blurImg = cvCreateImage( cvSize( width , height ) , IPL_DEPTH_8U , 3 );
	...
	cvResizeWindow("Blurred Image", width , height );
	...
	for( int imgCount = imgStartNum ; imgCount <= imgEndNum ; ++imgCount )
	{
		...
		unsigned char erc = blurImage( rgbImg , blurImg , blurWindowSize);
		if( erc != 0 )
		{
			printf( "\n\n Error Code = %d \n\n" , erc );
			getchar();
		...
		cvShowImage( "Blurred Image" , blurImg );
