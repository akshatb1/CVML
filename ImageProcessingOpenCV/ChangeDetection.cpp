...
int main( int argc , char** argv )
{
	...
	IplImage* changeImg = NULL;
	changeImg = cvCreateImage( cvSize( width , height ) , IPL_DEPTH_8U , 1 );
	// Allocate Memory for Eroded Output
	IplImage* erodeImg = NULL;
	erodeImg = cvCreateImage( cvSize( width , height ) , IPL_DEPTH_8U , 1 );
	//Allocate Memory for Dilated Output
	IplImage* dilatedImg = NULL;
	dilatedImg = cvCreateImage( cvSize( width , height ) , IPL_DEPTH_8U , 1 );

	// Create the Windows for Displaying Images
	cvNamedWindow( "frameData" , CV_WINDOW_AUTOSIZE );
	cvResizeWindow( "frameData" , width , height );
	cvNamedWindow( "Change Detection" , CV_WINDOW_AUTOSIZE );
	cvResizeWindow("Change Detection", width , height );
	cvNamedWindow( "Eroded Output" , CV_WINDOW_AUTOSIZE );
	cvResizeWindow("Eroded Output", width , height );
	cvNamedWindow( "Dilated Final Output" , CV_WINDOW_AUTOSIZE );
	cvResizeWindow("Dilate Final Output", width , height );
	...
	for( int imgCount = imgStartNum+1 ; imgCount <= imgEndNum ; ++imgCount )
	{
		...
		unsigned char erc = changeDetection( rgbImg , rgbImg_prev, changeImg , erodeImg, dilatedImg, threshold);
		if( erc != 0 )
		{
			printf( "\n\n Error Code = %d \n\n" , erc );
			getchar();
		...
		cvShowImage( "Change Detection" , changeImg );
		cvShowImage( "Eroded Output", erodeImg);
		cvShowImage( "Dilated Final Output", dilatedImg);
	...
	cvDestroyWindow( "Change Detection" );
	cvDestroyWindow( "Eroded Output");
	cvDestroyWindow( "Dilated Final Output");

	// Cleaning Memory : Images 
	cvReleaseImage( &changeImg );
	cvReleaseImage( &erodeImg);
	cvReleaseImage( &dilatedImg);
